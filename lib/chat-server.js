...
var playPoker = function(socket, io){
  ...
  socket.on('raise', function(data) {
    var toCall = data.game.currentBet - playerRooms[data.room][data.game.activePlayer].currentBet
    var toBet = toCall + 50;
    data.game.currentBet += 50;
	playerRooms[data.room][data.game.activePlayer].bet(toBet);
	data.game.numCalled = 1;
	checkCalls(data.game);
  })
